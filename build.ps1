...
& java -jar swagger-codegen-cli-2.4.5.jar generate -i .\documentai-api-swagger.json -l csharp -o client -c packageconfig.json
...
$csprojpath = Resolve-Path ./client/src/Cloudmersive.APIClient.NET.DocumentAI/Cloudmersive.APIClient.NET.DocumentAI.csproj
$csprojtestpath = Resolve-Path ./client/src/Cloudmersive.APIClient.NET.DocumentAI.Test/Cloudmersive.APIClient.NET.DocumentAI.Test.csproj
$nuspecpath = Resolve-Path ./client/src/Cloudmersive.APIClient.NET.DocumentAI/Cloudmersive.APIClient.NET.DocumentAI.nuspec
$slnpath = Resolve-Path ./client/Cloudmersive.APIClient.NET.DocumentAI.sln
...
(Get-Content $nuspecpath).replace('<description>A library generated from a Swagger doc</description>', "<description>Document AI APIs let you use AI and Machine Learning to extract data, categories, summaries and insight from free-form text documents.</description>") | Set-Content $nuspecpath
...
(Get-Content $nuspecpath).replace('<dependencies>', "<projectUrl>https://cloudmersive.com/document-ai-api</projectUrl><dependencies>") | Set-Content $nuspecpath
...
(Get-Content '.\client\src\Cloudmersive.APIClient.NET.DocumentAI\packages.config').replace('<package id="RestSharp" version="105.1.0" targetFramework="net45" developmentDependency="true" />', '<package id="RestSharp" version="106.6.10" targetFramework="net45" developmentDependency="true" />') | Set-Content '.\client\src\Cloudmersive.APIClient.NET.DocumentAI\packages.config'
(Get-Content '.\client\src\Cloudmersive.APIClient.NET.DocumentAI\Client\ApiClient.cs').replace('request.AddFile(param.Value.Name, param.Value.Writer, param.Value.FileName, param.Value.ContentType);', 'request.AddFile(param.Value.Name, param.Value.Writer, param.Value.FileName, param.Value.ContentLength, param.Value.ContentType);') | Set-Content '.\client\src\Cloudmersive.APIClient.NET.DocumentAI\Client\ApiClient.cs'
